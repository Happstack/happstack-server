...
{-
renderByteRangesBuilder :: ByteRange -> Blaze.Builder
...
renderByteRange = Blaze.toByteString . renderByteRangesBuilder
-}
isInOrderNonOverlapping :: Integer -- ^ Content-Length header value
                        -> ByteRanges    -- ^ ByteRanges we are checking
                        -> Maybe [(Integer, Integer)] -- ^ valid ranges
isInOrderNonOverlapping _ []            = Just []
isInOrderNonOverlapping len br@[ByteRangeFromTo f t]   | f < len     = Just [(f, min t (len - 1))]
isInOrderNonOverlapping len br@[ByteRangeFrom f]       | f < len     = Just [(f, len - 1)]
isInOrderNonOverlapping len br@[ByteRangeSuffix count] | count < len = Just [(len - count, len - 1)]
isInOrderNonOverlapping len br@[ByteRangeSuffix count] | otherwise   = Nothing -- must be overlapping since we only call this when there are two or more byteranges?
isInOrderNonOverlapping len (br0:br1:brs) =
    (isOk br0 br1) (isInOrderNonOverlapping len (br1 : brs))
    where
      isOk (ByteRangeFrom _) _   = (const Nothing) -- ByteRangeFrom extends to the end, so there should not be any ranges after it
      isOk (ByteRangeSuffix _) _ = (const Nothing) -- ByteRangeSuffix extends to the end so there should not be any ranges after it
      isOk (ByteRangeFromTo f1 t1) (ByteRangeFromTo f2 t2)
           | (t1 < f2) && (f1 < len) = fmap ((f1, min t1 len) :)
           | (t1 >= f2) = const Nothing
      isOk (ByteRangeFromTo f1 t1) (ByteRangeSuffix suffix)
           | (t1 < (len - suffix)) && (f1 < len) = fmap ((f1, min t1 len) : )
           | (t1 >= (len - suffix)) = const Nothing
      isOk (ByteRangeFromTo f1 t1) (ByteRangeFrom f2)
           | (t1 < f2) && (f1 < len) = fmap ((f1, min t1 len ) : )
           | (t1 >= f2) = const Nothing
      isOk _ _ = id
                                                               ...
                                                               | otherwise ->
                                                                  setHeaderBS (original hContentLength) (C.pack $ show count) $
                                                                   setHeaderBS (original hContentRange)
                                                                               (C.pack "bytes " <> renderByteRange (ByteRangeFromTo (contentLength - count) (contentLength - 1)) <> (C.pack "/") <> (C.pack $ show contentLength)) $
                                                                     response { rsCode = 206
                                                         ...
                                                         | (offset == 0) -> -- I am not sure what circumstances would result in offset being non-zero at this point, so I am reluctant to
                                                             ...
                                                             case br of
                                                               (ByteRangeFromTo f t) | t <= count ->
                                                                setHeaderBS (original hContentLength) (C.pack $ show (1 + t - f)) $
                                                                 setHeaderBS (original hContentRange) (C.pack "bytes " <> renderByteRange br <> (C.pack "/") <> (C.pack (show (count - offset)))) $
                                                                    response { rsCode = 206
                                                               ...
                                                               (ByteRangeSuffix suffix)
                                                                   ...
                                                                   | otherwise ->
                                                                     setHeaderBS (original hContentLength) (C.pack $ show suffix) $
                                                                      setHeaderBS (original hContentRange) (C.pack "bytes " <> renderByteRange (ByteRangeFromTo (count - suffix) (count - 1)) <> (C.pack "/") <> (C.pack (show (count - offset)))) $
                                                                       response { rsCode = 206
                                                               ...
                                                               (ByteRangeFrom f)
                                                                   | f < count ->
                                                                       setHeaderBS (original hContentLength) (C.pack $ show (count - f)) $
                                                                        setHeaderBS (original hContentRange) (C.pack "bytes " <> renderByteRange (ByteRangeFromTo f (count - 1)) <> (C.pack "/") <> (C.pack (show (count - offset)))) $
                                                                       response { rsCode = 206
